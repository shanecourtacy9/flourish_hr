...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.authService.getProfile().subscribe((corporateAdmin) => {
      ...
      this.f.membershipExpireAt.setValue(
        corporateAdmin.company
          ? corporateAdmin.company?.["configs"]["membershipExpireAt"]
   ...
   */
  updateProfile() {
    let changedValues = DirtyValues.getDirtyValues(this.corporateAdminForm);
    if (Object.keys(changedValues).length > 0) {
      this.authService.updateProfile(changedValues).subscribe((message) => {
