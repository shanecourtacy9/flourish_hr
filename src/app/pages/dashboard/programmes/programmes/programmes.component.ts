...
export class ProgrammesComponent implements OnInit {
  ...
  constructor(
    ...
    this.programmesDatasource = new ProgrammesDatasource(this.programmesService)
  ...
  ngAfterViewInit(): void {
    ...
    if (this.isMember) {
        ...
        .pipe(
          ...
          if ((dateRange['startDate'] && dateRange['endDate']) || (!dateRange['startDate'] && !dateRange['endDate'])) {
            if (dateRange['startDate'] !== null) {
              ...
              this.getProgrammes(startDate, endDate, this.sort.direction, this.paginator.pageIndex, this.paginator.pageSize)
            } else {
              this.getProgrammes('', '', this.sort.direction, this.paginator.pageIndex, this.paginator.pageSize)
            }
          }
        })
      ...
      this.paginator.page
        .subscribe(() => {
          this.getProgrammes(this.f['startDate'].value, this.f['endDate'].value, this.sort.direction, this.paginator.pageIndex, this.paginator.pageSize);
        })
    }
  }
  ...
  editProgramme(programme) {
    ...
    this.router.navigate([`/dashboard/programmes/edit/${programme._id}`], navigationState)
  }
  /**
    ...
    if (programmes.length > 0) {
      let convertedProgrammes = programmes.reduce((acc, current) => {
        ...
        let convertedProgramme = {
          ...
          startTime: dateFns.format(new Date(dateFns.parseISO(current.startDate)), 'hh:mm a'),
          endTime: dateFns.format(new Date(dateFns.parseISO(current.endDate)), 'hh:mm a'),
          ...current
        };
  ...
  openConfirmDialog(programme) {
    ...
    const dialogData = new ConfirmDialogModel("Confirm Delete programme", message);
  ...
  recruitUsrs(row) {
    ...
    const dialogData = new ConfirmDialogModel("Confirm send emails to all", message);
    ...
    dialogRef.afterClosed()
      .subscribe(dialogResult => {
        if (dialogResult) {
          row.isClicked = true
          this.programmesService.sendRecruitEmails(row)
            .subscribe(res => {
              row.isClicked = false
              if (res) {
  ...
  reminderUsers(row) {
    ...
    const dialogData = new ConfirmDialogModel("Confirm send emails to registered users", message);
    ...
    dialogRef.afterClosed()
      .subscribe(dialogResult => {
        if (dialogResult) {
          row.isClicked = true
          this.programmesService.sendReminderEmails(row)
            .subscribe(res => {
              row.isClicked = false
              if (res) {
                this.snackBar.open('send successfully', '', { duration: 2000 });
