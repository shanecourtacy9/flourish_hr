...
export class AuthService {
  ...
  signin(credentialInfo?: { email: string; password: string }) {
    return this.http.post(`${URL}/signin`, credentialInfo).pipe(
      tap((tokens) => {
  ...
  sendOtp(credentialInfo?: { email: string; password: string }) {
    return this.http.post(`${URL}/sendOtp`, credentialInfo).pipe(
      map((res) => res["token"]),
      catchError((err) => {
        throw err;
      }),
  ...
  verifyOtp(credentialInfo?: {
    ...
    return this.http.post(`${URL}/verifyOtp`, credentialInfo).pipe(
      tap((tokens) => {
        this.currentAccessToken = tokens["access_token"];
        let user = this.jwtHelper.decodeToken(this.currentAccessToken);
        this.userTypeSubject.next(user.userType);
  ...
  getProfile() {
    ...
    return this.http.get(`${URL}/profile`).pipe(
  ...
  updateProfile(properties) {
    return this.http.put(`${URL}/profile`, properties).pipe(
      map((res) => res["message"]),
      catchError((err) => {
        throw err;
      })
  ...
  changePassword(properties) {
    return this.http.put(`${URL}/profile/changePassword`, properties).pipe(
      map((res) => res["message"]),
      catchError((err) => {
        throw err;
      })
  ...
  forgetPassword(email) {
    return this.http.put(`${URL}/forgetPassword`, email).pipe(
      map((res) => res["message"]),
      catchError((err) => {
        throw err;
      })
